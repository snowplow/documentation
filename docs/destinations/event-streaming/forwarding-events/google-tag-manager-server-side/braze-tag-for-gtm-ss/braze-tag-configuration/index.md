...
This section includes the mapping rules that concern a Snowplow event as claimed by the [Snowplow Client](/docs/destinations/event-streaming/forwarding-events/google-tag-manager-server-side/snowplow-client-for-gtm-ss/index.md):
...
This section is available only if the [Include Self Describing event](/docs/destinations/event-streaming/forwarding-events/google-tag-manager-server-side/braze-tag-for-gtm-ss/braze-tag-configuration/#snowplow-self-describing-event) option is enabled. Using this drop-down menu you can indicate the location where Snowplow Self Describing event properties should be added under Braze event properties. The available options are:
