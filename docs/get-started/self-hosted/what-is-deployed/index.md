...
This is an application that receives raw Snowplow events over HTTP(S), serializes them, and then writes them to Kinesis (on AWS), Pub/Sub (on GCP) or Kafka / Event Hubs (on Azure). More details can be found [here](/docs/api-reference/stream-collector/index.md).
...
If you selected Redshift, Snowflake or Databricks as your destination, the loader will use an extra SQS stream internally as explained in the [reference documentation](/docs/api-reference/loaders-storage-targets/snowplow-rdb-loader/index.md?cloud=aws).
