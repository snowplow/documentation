...
Any custom derived model must build only to session-level or _lower_ because of the [incremental sessionization](/docs/data-models/models-dbt/package-mechanics/incremental-processing/index.md) of our packages. For example you can't build a table that aggregates per day using the standard sessionisation as not all events from a given day are always processed in the same run, at least not on any of the [this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md), see the [Daily Aggregate Table](/docs/data-models/models-dbt/dbt-custom-models/examples/daily-aggregate-table/index.md) for how to do this.
...
The skeleton of the model would be to make an incremental model, ensure we add the [optimized upsert](/docs/data-models/models-dbt/package-mechanics/optimized-upserts/index.md), [tag](/docs/data-models/models-dbt/dbt-custom-models/index.md#tagging-models) the model, checking for new events, and selecting from [events this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md#events-this-run).
...
Follow the steps to [backfill](/docs/data-models/models-dbt/dbt-operation/backfilling/index.md) the model. Make sure to add any additional config for optimizing your tables based on your warehouse.
