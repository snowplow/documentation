...
Now that you have a working pipeline you can *optionally* configure your Collector and Iglu Server to have a HTTPS enabled endpoint. This might be required in some cases to be able to track events to your pipeline on strictly SSL only websites as well as enabling [first-party tracking](/docs/collecting-data/first-party-tracking/) by being able to put the Collector endpoint on the same sub-domain as your website.
...
4. Copy the issued certificates `ARN` and copy it into your `terraform.tfvars` file under `ssl_information.certificate_arn`
...
You should now be able to access your service over HTTPS - validate this by going to your newly setup endpoint in a browser and verifying that a valid response comes back and that the SSL certificate is valid.
...
2. Select the `Classic Certificates` tab and then press `Create SSL Certificate`
  ...
  - `Create mode` allows you to either supply your own certificate or create a `Google-managed` - we would generally recommend the later for ease of implementation and updates
...
Once the certificate is issued you should be able to access your service over HTTPS - validate this by going to your newly setup endpoint in a browser and verifying that a valid response comes back and that the SSL certificate is valid.

:::info

Its worth noting that GCP Managed Certificates can take up to 24 hours to be provisioned successfully so it might take a while before you can access your service over HTTPS.
...
Azure doesn't have a Certificate Management layer like AWS & GCP so to follow this guide you will need to have bought your own valid SSL certificate for use which covers the domain you want to bind onto.

1. [Convert your SSL Certificate](https://github.com/snowplow-devops/terraform-azurerm-lb?tab=readme-ov-file#adding-a-custom-certificate) into the correct format needed for the Azure Application Load Balancer
  - Ultimately this requires you to convert your certificate into a `pkcs12` format
