...
SQS is used to queue any message that Stream Collector failed to send to Kinesis. The [Snowbridge application](/docs/destinations/event-streaming/forwarding-events/snowbridge/index.md) can then read the messages from SQS and write them to Kinesis once Kinesis is ready. In the event of any AWS API glitches, there is a retry mechanism which retries sending to the SQS queue 10 times.
