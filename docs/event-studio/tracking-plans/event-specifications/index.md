...
Once an event specification is ready, you can publish it to make it active in the pipeline. Publishing enables [event specification inference](/docs/event-studio/tracking-plans/event-specification-inference/index.md), which automatically matches incoming events to your published specifications and surfaces volume and observability data in the Console. It also enables [event specification validation](/docs/event-studio/tracking-plans/event-specification-validation/index.md) for events that arrive with a specification entity attached, attaching a validation entity to the event when it does not conform to the specification's rules.
