...
| `collector.cookie.clientCookieName` (since 3.4.0) | Optional. Default: not set.  If set to a cookie name (e.g. `sp_client`) then the collector sets an extra cookie with `httpOnly=false`, whose value is the same as the main cookie. For advanced use cases, this allows Javascript on the client side to read the Snowplow user ID. |
