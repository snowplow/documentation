...
:::tip
A good practice is to coalesce the `domain_userid` and the `userId` property in the client session context entity to make sure that the value can come from either place. This is also what our dbt packages do.
:::
...
Sessions have a configurable inactivity timeout.
