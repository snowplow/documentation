...
**NOTE:** Deduplication is currently only available in the [Spark transformer](/docs/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/spark-transformer/).
...
This is less of a concern with [wide row format](/docs/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/#wide-row-format) loading into **Snowflake**, where it's easier to deduplicate during the data modeling step in the warehouse.
...
To enable this functionality, you need to have the [Event Fingerprint Enrichment](/docs/enriching-your-data/available-enrichments/event-fingerprint-enrichment/) enabled in Enrich. This will correctly populate the `event_fingerprint` property. No changes are required in the transformer's own `config.hocon` file.
...
To avoid 'cold start' problems you may want to use the [`event-manifest-populator`](/docs/pipeline-components-and-applications/loaders-storage-targets/events-manifest-populator/) Spark job, which backfills the duplicates table with events from a specified point in time onwards.
