...
Follow the documentation for one of [our trackers](/docs/sources/collecting-from-own-applications/index.md) to implement some tracking code on your website or application.

You can then point it to `localhost:9090` where Micro is listening. For example, using the [Browser tracker](/docs/sources/collecting-from-own-applications/javascript-trackers/web-tracker/index.md):
...
Would you rather see the events visually? Open [http://localhost:9090/micro/ui](http://localhost:9090/micro/ui) in your browser. You might want to check [a few tips on how to use the UI](/docs/data-product-studio/data-quality/testing-debugging/snowplow-micro/ui/index.md).
...
Snowplow pipelines output data in the [_enriched TSV format_](/docs/fundamentals/canonical-event/understanding-the-enriched-tsv-format/index.md). Typically, this is picked up by one of our [loaders](/docs/destinations/loaders-warehouses/storage-options/index.md) or by tools such as [Snowbridge](/docs/destinations/event-streaming/forwarding-events/snowbridge/index.md).
