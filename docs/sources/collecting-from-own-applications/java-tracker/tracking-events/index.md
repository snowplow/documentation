...
`EcommerceTransaction` and `EcommerceTransactionItem` events are legacy primitive events. We recommend instead designing your own `SelfDescribing` events plus [context entities](/docs/sources/collecting-from-own-applications/java-tracker/custom-tracking-using-schemas/index.md) for eCommerce tracking. 
