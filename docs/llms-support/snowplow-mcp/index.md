...
The assistant can perform any action the key allows. When you create the key, grant only the permissions the assistant needs. For read-only use, select the **View** level on each feature. Keys marked **Global admin** have full admin permissions, so create a new key rather than reusing one. Store the key securely and only use this method in environments you trust.
:::
