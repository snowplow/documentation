...
The environment infers a specification for events that arrive without an `event_specification` entity. Inference considers one version of each specification: the draft when there is one, and the highest published version otherwise. See [event specification inference](/docs/event-studio/tracking-plans/event-specification-inference/index.md) for the matching rules.
