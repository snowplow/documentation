...
You may want to set the snowplow `aid=` parameter in your URL query string to the company for which this webhook has been configured; this is the `app_id` parameter taken from the [Snowplow Tracker Protocol](/docs/snowplow-events/index.md), however this is optional. The company is also contained in the received messages.
...
Supported platform codes can again be found in the [Snowplow Tracker Protocol](/docs/snowplow-events/index.md); if not set, then the value for `platform` will default to `srv` for a server-side application.
