...
Snowplow pipelines are "non-lossy", this means if something is wrong with an event during any part of the pipeline, the event is stored in a separate storage environment rather than just discarded. See the [failed events section](/docs/migrated/managing-data-quality/understanding-failed-events/) for more information on the types of failures that may occur.
