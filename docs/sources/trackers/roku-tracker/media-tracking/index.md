...
For backwards compatibility with v0.2.0 and earlier, you can also use `enableVideoTracking` and pass the node via `video` instead of `media`.
`enableAudioTracking` and `audio` will work as well; the tracking will work with any combination.

Media tracking has [multiple versions](/docs/sources/trackers/javascript-trackers/web-tracker/tracking-events/media/) of schema available.
The Roku tracker supports both; but in versions prior to v0.3.0, only version 1 schemas were supported and are the default for backwards compatibility.
...
The options differ by version; but version 2 will accept the options from version 1 if it can't find its equivalent settings.
...
To stop tracking events from the node, set the `disableMediaTracking` property. The tracker will then stop observing and tracking events from the node.
The property should be set with an `roAssociativeArray` with exactly one attribute, `media` (or `audio`/`video`), like so:
