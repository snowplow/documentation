...
If you are running Stream Collector on AWS and using Kinesis, then you might want to [set up an SQS buffer](/docs/getting-started-on-snowplow-open-source/setup-snowplow-on-aws/setup-the-snowplow-collector/set-up-an-sqs-buffer/index.md) as a backup when Kinesis does not auto-scale fast enough during extreme traffic spikes.
