...
| `patchesAllowed` | Optional. Default: `false`. If `true`, schemas sent to the `/api/schemas` endpoint will overwrite existing ones rather than be skipped if a schema with the same key already exists. |
