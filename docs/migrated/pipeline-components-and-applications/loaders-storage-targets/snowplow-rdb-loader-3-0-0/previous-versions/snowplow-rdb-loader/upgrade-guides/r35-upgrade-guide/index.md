...
1. S3DistCp, copying enriched data sunk by [S3 Loader](/docs/migrated/pipeline-components-and-applications/loaders-storage-targets/s3-loader/), from S3 sink bucket (same as "enriched stream bucket") into _enriched data lake_ (aka shredder input, similar as previously known "enriched archive")
...
We recommend to use either [Dataflow Runner](/docs/migrated/pipeline-components-and-applications/dataflow-runner/) or boto3 script to launch scheduled S3DistCp and Shredder jobs. Here's an example of a Dataflow Runner playbook:
...
CLI arguments also have changed. Both applications now accept only `--iglu-config` with base64-encoded string representing [Iglu Resolver JSON](/docs/migrated/pipeline-components-and-applications/iglu/iglu-resolver/) and `--config` with base64-encoded above HOCON. Loader also accepts `--dry-run` flag.
