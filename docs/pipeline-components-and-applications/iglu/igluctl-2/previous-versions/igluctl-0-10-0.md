...
Igluctl lets you skip the below checks:
...
If you’re not a Snowplow Platform user, don’t use [Self-describing Schema](/docs/pipeline-components-and-applications/iglu/common-architecture/self-describing-json-schemas.md) or just don’t want anything Iglu-specific, you can produce raw DDL:
...
- `host` - Iglu Server host name or IP address with optional port and endpoint. It should conform pattern `host:port/path` (or just `host`) **without** http:// prefix.
- `apikey` - master API key, used to create temporary write and read keys
