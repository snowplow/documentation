...
date: "2021-08-06"
sidebar_position: 10
description: "Track platform context with device information and application context with app version using React Native tracker v2 SDK."
keywords: ["react native tracker v2 platform context", "device information", "app version tracking"]
---
...
Please [follow the documentation for the iOS tracker](/docs/sources/mobile-trackers/installation-and-set-up/index.md) to call the `Snowplow.createTracker()` function inside the `application(_:didFinishLaunchingWithOptions:)` method. Make sure to pass the `TrackerConfiguration.advertisingIdentifierRetriever` callback that retrieves the ID as [described here](/docs/sources/mobile-trackers/tracking-events/platform-and-application-context/index.md?platform=ios#identifier-for-advertisers-idfaaaid).
