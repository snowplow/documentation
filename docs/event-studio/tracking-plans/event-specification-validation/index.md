...
1. **Event constraints**: property-level rules defined on the event data structure, such as expected values or ranges
...
The `add_to_cart` data structure schema permits `currency` to be either `"USD"` or `"EUR"`. The specification narrows it to `"USD"`, so the event constraint catches values that the data structure schema would otherwise accept.
...
An `add_to_cart` event arrives with an event payload where `currency = "EUR"`. The data structure schema accepts the value, but the specification's event constraint does not. The pipeline attaches a validation entity with `isValid: false` and an error identifying the event schema and the violated rule.
