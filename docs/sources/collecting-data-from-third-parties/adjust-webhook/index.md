...
On the Snowplow side, we will use the [Iglu webhook adapter](/docs/sources/collecting-data-from-third-parties/iglu-webhook/index.md) to ensure that the event is correctly received and processed. We will create a schema for the event that matches the structure of the data sent from Adjust.
