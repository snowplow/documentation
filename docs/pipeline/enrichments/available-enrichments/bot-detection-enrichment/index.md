...
It is safe to enable all sources (`useYauaa`, `useIab`, `useAsnLookups`, `useClientSideDetection`) even if some of the underlying enrichments or plugins are not in use. If a contributing source is not enabled or its entity is not present, that source is silently skipped.
