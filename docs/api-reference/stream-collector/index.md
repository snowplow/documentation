...
The Stream Collector receives raw Snowplow events sent over HTTP by [trackers](/docs/sources/collecting-from-own-applications/index.md) or [webhooks](/docs/sources/collecting-data-from-third-parties/index.md). It serializes them to a [Thrift](http://thrift.apache.org/) record format, and then writes them to a sink. Currently supported sinks are:
