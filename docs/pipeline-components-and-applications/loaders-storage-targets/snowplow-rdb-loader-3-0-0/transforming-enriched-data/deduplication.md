...
**NOTE:** Deduplication is currently only available in the [Spark transformer](/docs/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/spark-transformer.md).
...
To avoid 'cold start' problems you may want to use the [`event-manifest-populator`](/docs/pipeline-components-and-applications/loaders-storage-targets/events-manifest-populator.md) Spark job, which backfills the duplicates table with events from a specified point in time onwards.
