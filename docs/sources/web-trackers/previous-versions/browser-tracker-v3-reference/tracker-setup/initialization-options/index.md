...
Anonymous tracking can be toggled on and off. The methods to control this behavior are described [here](/docs/sources/web-trackers/previous-versions/browser-tracker-v3-reference/tracker-setup/additional-options/index.md).
...
See also [How the Tracker uses `localStorage`](/docs/sources/web-trackers/cookies-and-local-storage/index.md) for an explanation of how the tracker can later recover and send unsent events.
