...
These examples show the configuration file and expected outcome for two example schemas.
...
/**
 * Track a self-describing event for WebPage.
 */
export function trackWebPage<T extends {} = any>(
  ...
  trackSelfDescribingEvent({ event, context, timestamp }, trackers);
}

/**
 * Track a self-describing event for Product.
 */
...
The examples below demonstrate how to use Snowtype's generated code. They assume you generated code for the `web_page` and `product` schemas.
...
## Generate event specification instructions as Markdown
