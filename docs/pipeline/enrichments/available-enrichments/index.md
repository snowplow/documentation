...
The order of enrichments is hard-coded and can't be changed. The table below lists the available enrichments **in the order they're executed**.

:::note[Single instance]

You can only configure one instance of each enrichment.

:::

| Enrichment                                                                                                               | Description                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| [IAB](/docs/pipeline/enrichments/available-enrichments/iab-enrichment/index.md)                                          | Detect robots or spiders via the IAB/ABC International Spiders and Bots List.                                                                    |
...
| [UA Parser (legacy)](/docs/pipeline/enrichments/available-enrichments/ua-parser-enrichment/index.md)                     | Not recommended. Parse the user agent string and attach an entity with detailed user agent information. YAUAA enrichment is preferred.                            |
| [Currency Conversion (legacy)](/docs/pipeline/enrichments/available-enrichments/currency-conversion-enrichment/index.md) | Not recommended. Convert the values of legacy transaction events to a specified base currency using Open Exchange Rates API.                     |
...
| [Campaign Attribution](/docs/pipeline/enrichments/available-enrichments/campaign-attribution-enrichment/index.md)        | Choose how query string parameters map to marketing campaign fields, e.g. `mkt_medium`.                                                          |
| [Cross Navigation](/docs/pipeline/enrichments/available-enrichments/cross-navigation-enrichment/index.md)                | Parse the extended cross navigation format in `_sp` query string parameter and attach the `cross_navigation` entity to an event.                 |
