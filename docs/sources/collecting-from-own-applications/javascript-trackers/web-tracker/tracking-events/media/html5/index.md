...
This plugin enables the automatic tracking of HTML media elements (`<video>` and `<audio>`) on a webpage, using the [Snowplow Media Plugin](/docs/sources/collecting-from-own-applications/javascript-trackers/web-tracker/tracking-events/media/snowplow/index.md).
...
| `config.context` | [`DynamicContext`](/docs/sources/collecting-from-own-applications/javascript-trackers/web-tracker/custom-tracking-using-schemas/global-context/#global-contexts-methods) | Dynamic contexts attached to each tracking event. | `undefined` |
