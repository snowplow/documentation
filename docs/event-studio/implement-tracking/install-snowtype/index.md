...
Snowtype needs a Console API key to read your event specifications and data structures. [Create an API key](/docs/account-management/index.md#create-an-api-key) in Console under **Settings** > **Manage organization** > **View and manage API keys**. Snowtype only reads data, so the key needs the _View_ permission on tracking plans and data structures, and nothing else.
