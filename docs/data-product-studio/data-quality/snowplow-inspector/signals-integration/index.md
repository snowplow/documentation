...
- Discovery: log into Console via the extension, and the extension will detect Signals instances for any organizations you have access to; for full functionality you will need to add API credentials for each organization in the extension options
...
After connecting to Signals, Inspector will use the Signals API to discover any defined [attribute keys](/docs/signals/concepts/index.md#attribute-keys), and the definitions for your attribute groups.
It uses these definitions with any events it has observed in the debugger to build a set of attribute keys.

For example, by default Signals includes `domain_userid` as an attribute key.
...
By default, Inspector only fetches the highest version of each attribute group.
