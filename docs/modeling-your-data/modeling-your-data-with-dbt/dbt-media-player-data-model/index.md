...
**The package can be found in the [snowplow/dbt-snowplow-media-player repo](https://github.com/snowplow/dbt-snowplow-media-player), with the full model doc site hosted [here](https://snowplow.github.io/dbt-snowplow-media-player/#!/overview/snowplow_media_player).**
...
**Note the models site is linked to the latest release of the package. If you are not using the latest release, [generate and serve](https://docs.getdbt.com/reference/commands/cmd-docs#dbt-docs-serve) the doc site locally for accurate documentation.**
...
The package is built on top of the [dbt-snowplow-web package](/docs/modeling-your-data/modeling-your-data-with-dbt/dbt-web-data-model/) taking that as a basis to carry out the incremental update. It is designed to be run together with the web model in a similar manner to how a custom module would run.
