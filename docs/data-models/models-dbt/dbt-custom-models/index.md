...
2. **Incremental models**: these models build from the existing (or custom) [this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md#other-this-run-tables) tables(s) and are incrementally upserted to each run. This could include things like a custom sessions table with additional user information. These are the most common types of custom models.
3. **This run models**: these models build from the [events this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md#events-this-run) table that contains all columns for events being (re)processed in the run. These can be used to feed custom incremental models.

Alternatively, if you wish to only use the events this run logic and build everything on top of this yourself, you may be better looking at the [using different sessionisation example](/docs/data-models/models-dbt/dbt-custom-models/examples/using-different-sessionisation/index.md) for how to set up a new package from scratch.

The best way to learn is by doing, so check out the [Examples](/docs/data-models/models-dbt/dbt-custom-models/examples/index.md) section for specific custom models.
...
- Redshift/Postgres users: be careful about joining mutli-valued entities to any events table. Deduplication logic needs to be applied to avoid many-to-many joins. See the [using multi-valued entities example](/docs/data-models/models-dbt/dbt-custom-models/examples/using-mulit-valued-entities/index.md) for how to manage this.
...
### Tagging Models
Any incremental model, or one that builds from the events this run table, in the package requires tagging so that it can be tracked in the manifest table as part of the [incremental sessionisation logic](/docs/data-models/models-dbt/package-mechanics/incremental-processing/index.md). If you do not correctly tag the model you may get an error, or your package may get stuck in State 2.
...
To make use of our [efficient upsert](/docs/data-models/models-dbt/package-mechanics/optimized-upserts/index.md) you should add `snowplow_optimize=true` to your custom model config for incremental models, setting an appropriate `upsert_date_key` and `unique_id`.
...
See our page on [backfilling](/docs/data-models/models-dbt/dbt-operation/backfilling/index.md).

### Refreshing Custom Models
See our page on [full or partial refreshes](/docs/data-models/models-dbt/dbt-operation/full-or-partial-refreshes/index.md).
...
:::danger
 Do **NOT** just use `--exclude` on the retired models from your job in production. Currently the packages is unable to identify which models are due to be executed in a given run. As a result, if you exclude a model the package will get stuck in [State 3](/docs/data-models/models-dbt/package-mechanics/incremental-processing/index.md#state-3-models-out-of-sync) and continue to attempt to sync your excluded with the remaining models.
:::
