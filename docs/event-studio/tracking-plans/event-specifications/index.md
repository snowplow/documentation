...
Once an event specification is ready, you can publish it to make it active in the pipeline. Publishing enables two features: [event specification inference](/docs/event-studio/tracking-plans/event-specification-inference/index.md) and [event specification validation](/docs/event-studio/tracking-plans/event-specification-validation/index.md). Inference automatically matches incoming events to your published specifications, surfacing volume and observability data in the Console. Validation applies to events that arrive with a specification entity attached: if an event doesn't conform to the specification's instructions, a validation entity is attached to flag the issue.
