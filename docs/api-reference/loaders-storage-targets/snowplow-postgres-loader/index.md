...
The Postgres loader is not recommended for production use, especially with large data volumes. We recommend using a fully-fledged data warehouse like Databricks, Snowflake, BigQuery or Redshift, together with a [respective loader](/docs/destinations/loaders-warehouses/storage-options/index.md).

:::

:::tip Schemas in Postgres

For more information on how events are stored in Postgres, check the [mapping between Snowplow schemas and the corresponding Postgres column types](/docs/destinations/loaders-warehouses/schemas-in-warehouse/index.md?warehouse=postgres).
