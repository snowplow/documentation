...
**NOTE:** Deduplication is currently only available in the [Spark transformer](/docs/migrated/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/spark-transformer/).
...
If you are loading into **Redshift** (using [shredded data](/docs/migrated/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/#shredded-data)), we strongly recommend to deduplicate the data upstream of loading. Once duplicates are loaded into separate tables, table joins would create a Cartesian product.

This is less of a concern with [wide row format](/docs/migrated/pipeline-components-and-applications/loaders-storage-targets/snowplow-rdb-loader-3-0-0/transforming-enriched-data/#wide-row-format) loading into **Snowflake**, where it's easier to deduplicate during the data modeling step in the warehouse.
...
There is no transformer configuration required for this functionality: deduplication is performed automatically. It is optional but highly recommended to use the [Event Fingerprint Enrichment](/docs/migrated/enriching-your-data/available-enrichments/event-fingerprint-enrichment/) in Enrich in order to correctly populate the `event_fingerprint` property.
...
To avoid 'cold start' problems you may want to use the [`event-manifest-populator`](/docs/migrated/pipeline-components-and-applications/loaders-storage-targets/events-manifest-populator/) Spark job, which backfills the duplicates table with events from a specified point in time onwards.
