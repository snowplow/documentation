---
title: "Mobile data model for SQL Runner"
sidebar_label: "Mobile data model"
date: "2020-10-30"
...
![Diagram showing the three levels of the SQL Runner mobile data model: ENTITY (users) at the top, CYCLE (sessions) in the middle, and INTERACTION (screen views) at the bottom, connected by vertical lines.](images/Screenshot-2022-03-25-at-10.50.33.png)
...
![Comparison diagram showing the difference in aggregation structure between the Mobile model (top) and Web model (bottom). In the Mobile model, Atomic Events feeds into both Optional Modules and Screen Views in parallel, and both feed into Sessions, then Users. In the Web model, Atomic Events feeds linearly into Page Views, then Sessions, then Users.](images/web_vs_mobile.jpg)
...
- Snowplow [Android](/docs/sources/mobile-trackers/previous-versions/android-tracker/index.md) or [iOS](/docs/sources/mobile-trackers/previous-versions/objective-c-tracker/index.md) mobile tracker implemented with mobile events dataset present in a database.
...
![Full flowchart of the SQL Runner mobile data model. From Atomic Tables/Manifests, the base module produces mobile_events_staged, which feeds in parallel into the screen views module (producing mobile_screen_views and mobile_screen_views_staged) and the app errors optional module (producing mobile_app_errors and mobile_app_errors_staged). Both staged tables feed into the sessions module (producing mobile_sessions and mobile_sessions_userid_manifest_staged), which in turn feeds the users module (producing mobile_users). Each module has a main playbook and a complete playbook.](images/mobile_full_model_structure.jpg)
