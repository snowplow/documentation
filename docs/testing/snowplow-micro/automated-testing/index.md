...
`- name: Start Micro
    run: docker run --mount type=bind,source=$(pwd)/micro,destination=/config -p 9090:9090 snowplow/snowplow-micro:${versions.snowplowMicro} --config /config/micro.conf --iglu /config/iglu.json & \\
    working-directory: snowplow-micro-examples`
}</CodeBlock>
...
Using the [JavaScript Tracker](/docs/sources/web-trackers/index.md):
...
2. Environment variables.
    - Cypress allows for [many ways](https://docs.cypress.io/app/guides/environment-variables) to set environment variables. In this example we set them in the `plugins/index.js` [file](https://github.com/snowplow-incubator/snowplow-micro-examples/blob/main/testing/cypress/plugins/index.js).
