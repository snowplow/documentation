...
Each event has an associated context, which is composed of entities. The tracker attaches entities to the events based on the configuration, but you can attach your own [custom entities](/docs/sources/mobile-trackers/custom-tracking-using-schemas/index.md) as well.
...
Track the user viewing a screen within the application. This type of tracking is typically used when automatic [screen view tracking](/docs/sources/mobile-trackers/tracking-events/screen-tracking/index.md) is not suitable within your application.
