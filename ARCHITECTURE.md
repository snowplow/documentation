...
- **Registering the tools is only half of it.** Almost no browser implements the draft yet — it is behind a flag or an origin trial in Chrome and Edge, and absent everywhere else, agent browsers included — and the API itself has no notion of a connection, so a page that only registers tools is still invisible from outside itself. An agent reaches the page over an MCP transport, and with no server listening it reports the site as having no WebMCP at all. `src/webmcp/index.ts` therefore loads [`@mcp-b/global`](https://www.npmjs.com/package/@mcp-b/global), which supplies both halves: it installs the reference polyfill where the browser has no WebMCP, and in every browser it puts an MCP server and a tab transport in front of whatever context is there, native or polyfilled, mirroring registrations down so a native consumer still sees them. It runs unconditionally for that reason, not just as a fallback. The transport accepts connections from this origin only, which is where an injected client posts from. The runtime is a separate chunk loaded after hydration, but unlike the tools it loads for every reader, because an agent arrives unannounced. It declines to install on an insecure origin, so plain `http://` other than localhost still gets nothing.
