...
The tracker should be straightforward to use if you are familiar with Arduino development; any prior experience with Snowplow's [JavaScript tracker](/docs/sources/trackers/javascript-trackers/index.md) or Google Analytics (which has a similar API to Snowplow) is helpful but not necessary.
...
**2)** Read through the [rest of the documentation](/docs/sources/trackers/arduino-tracker/index.md) for the Arduino Tracker to learn exactly what tracking capabilities Snowplow can provide for your Arduino sketch.
