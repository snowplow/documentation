    ...
    <>
      <div className={styles.versionChecker}>
        <form className="DocSearch-Form">
          <label className="DocSearch-MagnifierLabel" htmlFor="dbt">dbt:</label><br/>
          <input className="DocSearch-Input" placeholder="e.g. 1.3.0" maxLength="15" type="text" id="dbt" name="dbt" onChange={() => setDbtVer(event.target.value)}/>
        </form>
        <form className="DocSearch-Form">
          <label className="DocSearch-MagnifierLabel" htmlFor="utils">dbt_utils:</label><br/>
          <input className="DocSearch-Input" placeholder="e.g. 1.0.0" maxLength="15" type="text" id="utils" name="utils" onChange={() => setutilsVer(event.target.value)}/>
        </form>
      </div>
      <br/>
        ...
        <tbody>
          {packageVersions.map(x =>
            <tr key={x.pkg}>
              ...
              <td>
                {x.maxVer ? <code><a href={`https://hub.getdbt.com/${x.pkg}/${x.maxVer}`}>{x.maxVer}</a></code> : 'No supported version'}
              </td>
              <td>
