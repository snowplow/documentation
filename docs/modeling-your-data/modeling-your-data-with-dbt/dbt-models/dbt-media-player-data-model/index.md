...
- **Media base:** Summarizes the key media player events and metrics of each media element on a media session level (or media_id and page/screen view if media session context is not tracked) which is considered as a base aggregation level for media interactions.
...
- **Media plays:** Removes impressions from the media base table to summarize media plays on a page_view by media_id level.
