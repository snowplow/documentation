...
Builds the index that powers the `/changelog/` listing page.

The changelog entries live in `changelog/<slug>/index.md` and are rendered as
...
* `path` — directory holding the article folders, relative to the site directory. Defaults to `changelog`.
* `routeBasePath` — URL prefix the permalinks are built from. Defaults to `changelog`.

The build fails if an article is missing `title`, `date`, or `update_type`, so a badly
migrated article cannot silently disappear from the listing.
