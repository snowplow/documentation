...
There should be no need to further de-duplicate the base [events this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md#events-this-run) table or any contexts for each row.
...
Any custom entities or self-describing events are able to be added to the [events this run](/docs/data-models/models-dbt/package-mechanics/this-run-tables/index.md#events-this-run) table, and are de-duped on by taking the earliest `collector_tstamp` record, by using the `snowplow__entities_or_sdes` variable in our package. See [modeling entities](/docs/data-models/models-dbt/package-features/modeling-entities/index.md) for more information and examples.
...
If you need to make use of a multi-valued entity in a custom model, see the [using multi-valued entities](/docs/data-models/models-dbt/dbt-custom-models/examples/using-mulit-valued-entities/index.md) custom model example page.
