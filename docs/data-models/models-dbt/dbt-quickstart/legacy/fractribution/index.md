...
- have `snowplow_web_page_views` derived table available as a source (generated by the [snowplow_web package](/docs/data-models/models-dbt/dbt-models/legacy/dbt-web-data-model/index.md))
- have a table with revenue data by users (`domain_userid`, `user_id`) that serves as another source for the fractribution calculations, you can choose either of the following options:
    ...
    - the `snowplow_ecommerce_transaction_interactions` derived table generated by the [snowplow_ecommerce](/docs/data-models/models-dbt/dbt-models/dbt-ecommerce-data-model/index.md) package
...
The package has some variables that need to be set before it can be run, you should edit these in your `dbt_project.yml` file. Further customization can be done via the variables listed in the [configuration page](/docs/data-models/models-dbt/dbt-configuration/legacy/fractribution/index.mdx).
...
- `snowplow__path_transforms`: A dictionary of path transforms and their arguments (see [Path Transform Options](/docs/data-models/models-dbt/dbt-models/legacy/dbt-fractribution-data-model/index.md#path-transform-options) section)
...
> _For more information about dealing with duplicates and the macro in this code, make sure to see our [deduplication docs](/docs/data-models/models-dbt/package-mechanics/deduplication/index.md)._
...
Execute the following either through your CLI, within dbt Cloud, or within [Snowplow BDP](/docs/data-models/running-data-models/dbt/index.md)
...
Run the adapter specific main fractribution script by specifying the conversion window start and end dates, and the attribution model (if you are not using the default `shapley`, see [here](/docs/data-models/models-dbt/dbt-models/legacy/dbt-fractribution-data-model/index.md#attribution-models) for more options). Example:
