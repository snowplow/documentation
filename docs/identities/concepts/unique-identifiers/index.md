...
sidebar_position: 20
date: "2025-02-25"
description: "How Snowplow Identities handles merge conflicts using unique identifiers to prevent incorrect profile merges."
...
In this example, Alice has been using ExampleCompany's website and mobile app. Identities has previously merged her web and mobile profiles into a single profile `sp_001`, linked to her `domain_userid`, `user_id`, and `apple_idfv` identifiers. The `user_id` identifier is marked as unique.
...
Alice shares her phone with a colleague, Bob. Bob logs into the ExampleCompany mobile app on Alice's phone. The event contains the device's `apple_idfv`, already linked to Alice's profile via the earlier merge, and Bob's `user_id`.
