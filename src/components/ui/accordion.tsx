...
const accordionItemVariants = cva('', {
  variants: {
    variant: {
      ...
      outline: 'border border-border border-solid rounded-lg bg-card px-4 py-0 [&>*]:my-0 [&_*]:my-0',
...
const accordionTriggerVariants = cva(
  'flex flex-1 items-center justify-between py-4 gap-2.5 text-foreground font-medium transition-all [&[data-state=open]>svg]:rotate-180 cursor-pointer hover:text-foreground/80 bg-transparent border-0 shadow-none [&>*]:m-0',
  {
    variants: {
      variant: {
        default: '',
        outline: 'bg-transparent border-0 shadow-none py-4 [&>*]:m-0',
        solid: '',
      },
      indicator: {
        arrow: '',
...
const accordionContentVariants = cva(
  'overflow-hidden text-sm text-accent-foreground transition-all data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down data-[state=closed]:opacity-0',
  {
    variants: {
      variant: {
        default: '',
