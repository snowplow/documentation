...
Snowplow trackers fire _events_, which are either `GET` or `POST` requests to a Snowplow collector. By appending an events parameters to these requests, trackers can pass data into the collector for processing by Snowplow.
...
This documentation should be used by anyone who would like to build their own tracker or who would like to understand the tracker payload in more detail. By utilising the parameters documented here, the author of a new tracker can be confident that his / her tracker will work with the rest of the Snowplow stack.
...
| **Parameter** | **Maps to**           | **Type** | **Description**  | **Example values**                     |
|---------------|-----------------------|----------|------------------|----------------------------------------|
...
Every line of data passed from the tracker should contain an event field (`e`) to denote the type of event being tracked (See the [events section](#events) for possible values).
...
All context to be attached to an event will be wrapped in an array by the user and passed to the tracker, which will wrap them in a self-describing JSON:
