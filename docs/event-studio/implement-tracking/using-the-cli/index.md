...
sidebar_label: "Snowtype CLI TODO delete"
...
On the first run, or when no `.snowtype-lock.json` file exists, Snowtype resolves the latest versions and creates a lock file next to the configuration file. On subsequent runs, `generate` reads the pinned versions from the lock file. To update versions, use the [`snowtype update`](#keeping-up-with-latest-updates) command.
...
- Types/Interfaces/Classes for each schema that relates to the Data Structures, Iglu Central Schemas and Event Specifications selected.
  - For each Event Specification [instruction set](/docs/event-studio/tracking-plans/index.md), a type for the adjusted schema is generated as well. The type/class will contain the Event Specification name as suffix to avoid conflicts.
- For each schema:
...
To add a local Data Structure repository to the code generation, either manually or through the `snowtype patch` command, you would only need the path/s to the repository/ies you have generated schemas using the [snowplow-cli](/docs/event-studio/programmatic-management/snowplow-cli/data-structures/index.md).
...
The `generate` command uses the versions pinned in the `.snowtype-lock.json` file. To check for newer versions, use the `snowtype update` command. By default, it checks all event specifications against the latest published versions.
...
# Check for the latest draft version
npx @snowplow/snowtype@latest update --latestDraft
```
...
While developing or testing, it might be useful to use [Snowplow Micro](/docs/testing/snowplow-micro/index.md) to validate against your new schemas in your development environment.
