...
![Data structure builder General Information form for a new data structure named checkout_funnel, with type set to Event, vendor set to snowplow.demo, and an autogenerated schema tracking URL of iglu:snowplow.demo/checkout_funnel/jsonschema/1-0-0. The Properties section shows no properties added yet, with an Add a property button.](images/data-structures-1.png)
...
![Create a Property dialog for the checkout_funnel data structure, with property name action_name, type String, string type set to Enumerated list with values add_to_cart, view_cart, checkout, proceed_to_pay, and pick_address, nullable set to No, and required set to Yes.](images/data-structures-2.png)
...
![Data structure builder edit page for cart_action showing two properties in the Properties table: type (string, required, must be one of add or remove) and propertytwo (integer, not required). The three-dot action menu is open on the type property, showing Edit property and Delete property options.](images/edit-data-structure.png)
...
![Data structure builder for cart_action after edits, with the schema tracking URL highlighted in yellow showing the version incremented to 2-0-0. A yellow banner at the bottom reads "Version increased to 2-0-0 — your current version is 1-1-0, based on your changes the new version will be 2-0-0" with a Clear changes button.](images/data-structure-version.png)
...
![Data structures listing filtered by the search term "user", showing five results with Development and Production version columns. Two entries have their Development version highlighted in yellow (1-0-2 and 1-0-1), indicating versions not yet promoted. The three-dot action menu is open on the first result, showing options including View data structure, Edit JSON, Edit with builder, Migrate to production, and Hide data structure.](images/image-8.png)
...
![Bottom of the data structures listing showing two entries (user and tenant_users) and a View hidden data structures link below the table.](images/image-9.png)
...
![Info banner reading "This data structure is locked because it is managed in an external repository and can't be edited from the console UI. Repository: https://myrepo.org" with Unlock and Learn more links.](images/locked-ds.png)
