...
Name the new extension something like "Snowplow Collector - Iglu POST". The "Iglu POST" here represents the fact we will be sending Zendesk events and contexts to [Iglu webhook adapter](/docs/migrated/collecting-data/collecting-data-from-third-parties/iglu-webhook/) via `POST` request.
